...
o.makeInstance = function(domainName, className, options, context, replayEvents) {
    ...
    // create instance (enabling instanseof)
    var instance = new definition.instanceConstructorFunction();

    // create base object
    var base = new definition.BaseConstructorFunction(context); 
    ...
    // replay
    //replayEvents(domainName, className, base);
...
o.provideClassDefinition = function(domainName, className) {
    ...
    if (typeof o.classes[domainName][className] === "undefined") {
        ...
        // providing instanseof functionality
        var instanceConstructorFunction = function(){};

        // load class text
        var text = o.load(o.composePath(domainName, className));

        // fetch the base object constructor function and meta data
        var meta = {};
        var BaseConstructorFunction = o.makeClass(text, meta);
        ...
        if (meta["includes"]) {
            ...
            BaseConstructorFunction = o.makeClass(text, {}); // {} in place of meta preserves the modifications
        }
        ...
        o.classes[domainName][className] = {
            meta: meta,
            BaseConstructorFunction: BaseConstructorFunction,
            instanceConstructorFunction: instanceConstructorFunction
        };
...
o.extendSuperClass = function(domainName, superClassName, BaseConstructorFunction, instanceConstructorFunction) {

        var superDefinition = o.provideClassDefinition(domainName, superClassName);

        // enabling instanceof functionality
        instanceConstructorFunction.prototype = new superDefinition.instanceConstructorFunction();

        var superBase = new superDefinition.BaseConstructorFunction();

        var superInstance = BaseConstructorFunction.prototype; // never redefine prototypes with freshly created objects because the constructor changes
...
o.instanceOf = function(domainName, className, instance) {

    // fetch class definition
    var definition = o.provideClassDefinition(domainName, className);

    return instance instanceof definition.instanceConstructorFunction;
